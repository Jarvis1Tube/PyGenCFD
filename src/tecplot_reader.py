...
def read_tecplot(filename, verbose):
    ...
    for line in datafile:
        ...
        if title_match >= 0:
            ...
            if verbose:
                print
                "found STRANDID or DATAPACKING, mode remains at " + str(mode)
        # mode remains unchanged, reading data
            ...
            if struct == 1 or len(data) < nodes:  # read data
                ...
                if struct == 0 and len(data) == nodes:
                    if verbose:
                        print(
                            "found last unstructured data line, overall length: "
                            + str(len(data))
                        )
                        print(type(data))
                        print
    ...
    for df in dfs:
        df.columns = [var.replace('"', "") for var in variables]

    dfs = [df.apply(pd.to_numeric, errors="coerce") for df in dfs]
...
if __name__ == "__main__":
    ...
    ALL_DAT = os.path.abspath("./generated/ALL.DAT")
    data = read_tecplot(ALL_DAT, verbose=False)[1][0]

    plt.plot(data['"X"'], (data['"Ta"'] - data['"T"']).abs())
    plt.show()
